...
static gboolean collect_path_row(const int32_t imgid,
                                 ...
                                 void *user_data)
{
  if(!g_str_has_prefix(image_path, "/testdb/nullable-timestamp/")) return TRUE;

  assert_true(imgid > 0);
  assert_int_equal(version, 0);
  ...
  return TRUE; // this test walks every row
}
...
static gboolean _count_two_rows(const int32_t imgid, const int64_t write_timestamp,
                                const gboolean write_timestamp_present, const int version,
                                const char *image_path, const int flags,
                                void *user_data)
{
  (void)imgid;
  (void)write_timestamp;
  (void)write_timestamp_present;
  (void)version;
  (void)image_path;
  (void)flags;
