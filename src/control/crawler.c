                             ...
                             void *user_data)
{
...

  {
    ...
    // step 1: check if the xmp is newer than our db entry, or the row carries no timestamp at
    // all -- a SQL NULL is not a zero to compare against, it is an image never written out.
    // FIXME: allow for a few seconds difference?
    if(!timestamp_present || timestamp < xmp_timestamp)
    {
