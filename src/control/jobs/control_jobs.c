...
static int32_t _control_export_job_run(dt_job_t *job)
{
  ...
  {
    if(module_storage->initialize_store(module_storage, module_data, &module_format, &fdata, &t, data->high_quality, data->upscale))
    {
  ...
  while(t && dt_control_job_get_state(job) != DT_JOB_STATE_CANCELLED)
  {
    ...
    if(image)
    {
      ...
      {
        dt_image_cache_read_release(darktable.image_cache, image);
        if(module_storage->store(module_storage, module_data, imgid, module_format, fdata, num, total, data->high_quality, data->upscale,
