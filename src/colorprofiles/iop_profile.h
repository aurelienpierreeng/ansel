 ...
 * used to sit here -- the only reason this header reached two layers up -- went with the
 * pipeline-facing half to develop/iop_profile.h.
 *
 * pixel/eaw.c, pixel/rgb_norms.h and pixel/colorequal_shared.h consume this half, which is
 * why it could not simply move up to develop/ with the rest.
 */
