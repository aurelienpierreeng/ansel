                                        ...
                                        int num_points)
{
  ...
  if(gui->creation)
  {
    ...
    float raw_center[2];
    dt_masks_gui_cursor_to_raw_norm(darktable.develop, gui, raw_center);
    x = raw_center[0];
    y = raw_center[1];
...
#ifdef _OPENMP
#pragma omp parallel for default(none) \
  dt_omp_firstprivate(ellpts, center, ta, tb, cosa, sina, ell) \
  if(ellpts > 100)
#endif
...
#ifdef _OPENMP
#pragma omp parallel for default(none) \
  dt_omp_firstprivate(grid, bbxm, bbym, bbw, endx, endy, w, buffer, points, w0, w1, inv_grid2) \
