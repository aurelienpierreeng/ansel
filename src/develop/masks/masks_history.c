...
gboolean dt_masks_forms_snapshot_ensure(GList **snapshot, dt_develop_t *dev)
{
  if(!IS_NULL_PTR(*snapshot)) return FALSE;
  *snapshot = dt_masks_snapshot_current_forms(dev, FALSE);
  return TRUE;
}
