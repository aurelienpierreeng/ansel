/*
   ...
   Copyright (C) 2013-2017 Jérémy Rosen.
...
static int import_images(lua_State *L)
{
  ...
  else
  {
    ...
    result = dt_image_import_lua(new_film.id, full_name);
    if(dt_film_is_empty(new_film.id)) dt_film_remove(new_film.id);
    dt_film_cleanup(&new_film);
    if(result == 0)
    {
...
{
  const int32_t img_id = luaL_checkinteger(L, -1);
  if(img_id < 1)
  {
...
static int collection_numindex(lua_State *L)
{
  ...
  int32_t imgid = dt_collection_get_nth(darktable.collection,index-1);
  if (imgid >0)
  {
    luaA_push(L, dt_lua_image_t, &imgid);
  } else {
