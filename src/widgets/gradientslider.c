/*
    ...
    Copyright (C) 2012-2016, 2018 Tobias Ellinghaus.
    ...
    Copyright (C) 2021 Harold le Clément de Saint-Marcq.
...
void dtgtk_gradient_slider_multivalue_set_value(GtkDarktableGradientSlider *gslider, gdouble value, gint pos)
{
  ...
  gslider->selected = gslider->positions == 1 ? 0 : -1;
  if(!dt_gui_widgets_suppressed()) g_signal_emit_by_name(G_OBJECT(gslider), "value-changed");
