...
static gchar *_header_markup(const dt_sv_logged_event_t *ev)
{
  ...
  gchar *out = g_strdup_printf(
      "<span font_family=\"IBM Plex Mono,monospace\">%9.3f</span>  <b>%-7s</b>  <span foreground=\"%s\">%-10s</span>  <b>%-16s</b>  "
      "<a href=\"%s\"><span font_family=\"IBM Plex Mono,monospace\">%s</span></a>  <span size=\"small\"><i>%s</i></span>",
...
static GtkWidget *_event_body_from(const char *json, GArray *links)
{
  ...
  if(links && links->len)
  {
    ...
    for(guint i = 0; i < links->len; i++)
    {
      ...
      g_string_append_printf(ls, "%s%s → <a href=\"%s\"><span font_family=\"IBM Plex Mono,monospace\">%s</span></a>", i ? "      " : "", lk->label,
                             lhx, lhx);
      g_free(lhx);
    }
  ...
  gchar *mono = g_strdup_printf("<span font_family=\"IBM Plex Mono,monospace\">%s</span>", escaped);
...
static void _rebuild_memory(void)
{
  ...
  for(guint i = 0; i < pe->len && i < MEMORY_MAX_ROWS; i++)
  {
    ...
    gchar *vram = (gpu && e->cl_count > 0)
                      ? g_strdup_printf("<span font_family=\"IBM Plex Mono,monospace\">+%8.2f MiB vRAM (%2d buf)</span>", e->cl_bytes / 1048576.0,
                                        e->cl_count)
                      : NULL;
    gchar *m = g_strdup_printf("<a href=\"%s\"><span font_family=\"IBM Plex Mono,monospace\">%s</span></a>  %.2f MiB  refs=%d hits=%d  <i>%s</i>", hx, hx,
