/*
    ...
    Copyright (C) 2010-2012, 2014-2016, 2020 Tobias Ellinghaus.
...
static gboolean _bulk_add_metadata(const GArray *rows)
{
...
static void _undo_metadata_free(gpointer data)
{
  ...
  g_list_free_full(metadata->before, dt_free_gpointer);
  metadata->before = NULL;
  g_list_free_full(metadata->after, dt_free_gpointer);
  metadata->after = NULL;
  dt_free(metadata);
}
                                  ...
                                  const gboolean undo_on, const gint action)
{
  ...
  for(const GList *images = imgs; images; images = g_list_next(images))
  {
    ...
    if(undo_on)
      pending_undo = g_list_append(pending_undo, undometadata);
    else
      _undo_metadata_free(undometadata);
  }
...
gboolean dt_metadata_clear(const GList *imgs, const gboolean undo_on)
{
  ...
  for(unsigned int i = 0; i < DT_METADATA_NUMBER; i++)
  {
    if(dt_metadata_get_type(i) == DT_METADATA_TYPE_INTERNAL) continue;
