 ...
 */
void _traverse_tree(GtkTreeModel *model, GtkTreeIter *iter)
{
  do {

    GtkTreePath *current = gtk_tree_model_get_path(model, iter);
    gchar *text;
    gtk_tree_model_get(model, iter, DT_LIB_COLLECT_COL_TEXT, &text, -1);
    g_free(text);
    gtk_tree_path_free(current);
    gtk_tree_store_set(GTK_TREE_STORE(model), iter, DT_LIB_COLLECT_COL_FONT, PANGO_WEIGHT_NORMAL, -1);

    if (gtk_tree_model_iter_has_child(model, iter))
    {
      GtkTreeIter child;
      if (gtk_tree_model_iter_children(model, &child, iter)) {
        _traverse_tree(model, &child);
      }
    }
 ...
 */
void _update_tree_view_nodes_font(GtkTreeSelection *selection)
{
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &path_iter))
  {
    ...
    {
      // we walk down the tree by recursion. If the library is made up of millions collections, the recursion
      // should be replaced by a stack-based iteration for better performance
      if (valid)
      {
        _traverse_tree(model, &it);
      }
      // the font of the selection is set in bold
      ...
      case DT_COLLECTION_PROP_CAMERA:; // camera
        ...
        while(sqlite3_step(stmt) == SQLITE_ROW)
        {
          ...
          gtk_list_store_set(GTK_LIST_STORE(model), &iter, DT_LIB_COLLECT_COL_TEXT, value,
                             ...
                             DT_LIB_COLLECT_COL_COUNT, count, DT_LIB_COLLECT_COL_FONT, PANGO_WEIGHT_NORMAL,
                             -1);
