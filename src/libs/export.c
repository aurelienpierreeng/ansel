...
static void _export_button_clicked(GtkWidget *widget, dt_lib_export_t *d)
{
  ...
  */
  ...
  /**
   * Upscale is activated and only one dimension is 0, we adjust it to ensure that the up-scaling will
   * happen. The null dimension is set to `MAX_ASPECT_RATIO` time the other dimension,
   * allowing for a ratio of max 1:100 exported images.
   */
  ...
  dt_control_export_t data = { .imgid_list = dt_act_on_get_images(TRUE, TRUE, TRUE),
                               ...
                               .total = g_list_length(data.imgid_list),

                               .high_quality = dt_conf_get_bool(CONFIG_PREFIX "high_quality_processing"),
                               .upscale = upscale,
