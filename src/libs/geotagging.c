/*
    ...
    Copyright (C) 2013-2016, 2021 Roman Lebedev.
    Copyright (C) 2013 Thomas Pryds.
    Copyright (C) 2014, 2019-2021 Pascal Obry.
    Copyright (C) 2017, 2020 parafin.
    ...
    Copyright (C) 2021-2022 Philippe Weyland.
...
static GDateTime *_get_image_datetime(dt_lib_module_t *self)
{
  ...
  if(imgid < 0) imgid = dt_act_on_get_first_image();
...
void gui_init(dt_lib_module_t *self)
{
  ...
  if(dt_conf_get_bool("/views/map/enable"))
  {
    ...
    GtkWidget *w = dt_ui_scroll_wrap(GTK_WIDGET(d->map.gpx_view), 100,
                                     "plugins/lighttable/geotagging/heighttracklist", DT_UI_RESIZE_DYNAMIC);
