/*
    ...
    Copyright (C) 2014, 2019 Ulrich Pegelow.
...
#pragma omp parallel for default(none) \
  dt_omp_firstprivate(fimg, height, in, roi, size, width, xtrans, nthreads, chunksize) \
    dt_omp_firstprivate(c) num_threads(nthreads) \
    schedule(static)
#endif
             ...
             void *const ovoid)
{
  ...
  else
  {
    ...
    if (filters != 9u)
      ...
      return wavelet_denoise_xtrans(ivoid, ovoid, roi_in, d, xtrans);
  }
  return 0;
}
...
{
  dt_iop_rawdenoise_data_t *d = (dt_iop_rawdenoise_data_t *)dt_calloc_align(sizeof(dt_iop_rawdenoise_data_t));
