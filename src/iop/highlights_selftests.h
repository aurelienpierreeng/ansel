...
// Parity self-tests of the harmonic-transposition OpenCL port. Each function runs one GPU
// stage and its CPU reference on the same synthetic scene and prints the largest absolute
// difference; all are gated behind an environment variable (HL_*_TEST=1) and run at most once
// per process, at the entry of the OpenCL processing path. They are the enforcement mechanism
// of the CPU/GPU parity contract: any change to a stage on either side must keep its self-test
// at its documented tolerance (1e-5 .. 1e-12 depending on the stage).
//
// This file is a textual include of highlights_harmonic.h (single translation unit): it is
// included AFTER every production definition, so the tests can call the static stage functions
// directly. It must not define anything the production code uses.
...
static void _sp_chol_cl_selftest(const int devid, void *gd_void, const dt_dev_pixelpipe_t *pipe)
{
  static int done = 0;
...
// process.
static void _region_blur_cl_selftest(const int devid, const dt_dev_pixelpipe_t *pipe)
{
  static int done = 0;
...
static void _cf_harmonic_fill_cl_selftest(const int devid, void *gd_void, const dt_dev_pixelpipe_t *pipe)
{
  static int done = 0;
  ...
  uint8_t *hole = (uint8_t *)dt_alloc_align(region_pixels);   // 1 = to fill
  uint8_t *anchor = (uint8_t *)dt_alloc_align(region_pixels); // 1 = trusted (GPU kernel convention)
...
static void _cf_joint_stage_cl_selftest(const int devid, void *gd_void, const dt_dev_pixelpipe_t *pipe)
{
  static int done = 0;
  ...
  uint8_t *anchor = (uint8_t *)dt_alloc_align(region_pixels);
  uint8_t *border = (uint8_t *)dt_alloc_align(region_pixels);
...
#undef OFF2
  ...
  {
    uint8_t *hole = (uint8_t *)dt_alloc_align(region_pixels);
    uint8_t *hole2 = (uint8_t *)dt_alloc_align(region_pixels);
    if(hole && hole2)
    {
      for(size_t i = 0; i < region_pixels; i++)
      {
...
static void _cf_stage_cl_selftest(const int devid, void *gd_void, const dt_dev_pixelpipe_t *pipe)
{
  static int done = 0;
  if(done || !getenv("HL_CFCL_TEST") || devid < 0) return;
  done = 1;
  ...
  uint8_t *border = (uint8_t *)dt_alloc_align(region_pixels);
  uint8_t *hole = (uint8_t *)dt_alloc_align(region_pixels);
  uint8_t *hole_border = (uint8_t *)dt_alloc_align(region_pixels);
  if(IS_NULL_PTR(estimate) || IS_NULL_PTR(valid) || IS_NULL_PTR(model_quality) || IS_NULL_PTR(estimate_gpu)
     || IS_NULL_PTR(input) || IS_NULL_PTR(moment1) || IS_NULL_PTR(moment2) || IS_NULL_PTR(moment3)
...
static void _hf_stage_cl_selftest(const int devid, void *gd_void, const dt_dev_pixelpipe_t *pipe)
{
  static int done = 0;
...
static void _selfdome_stage_cl_selftest(const int devid, void *gd_void, const dt_dev_pixelpipe_t *pipe)
{
  static int done = 0;
  ...
  // run the dumped real-region dome through both implementations and compare
  {
    ...
    if(dump_file)
    {
         ...
         && fread(&dump_downsample, sizeof(int), 1, dump_file) == 1)
      {
        ...
        uint8_t *dump_hole = (uint8_t *)dt_alloc_align(dump_pixels);
...
static void _joint_core_stage_cl_selftest(const int devid, void *gd_void, const dt_dev_pixelpipe_t *pipe)
{
  static int done = 0;
  ...
  // ---- CPU replica: production joint-core block ----
  {
    ...
    double *sp_rhs = sp_factor ? (double *)dt_alloc_align(sizeof(double) * sp_n_unknowns) : NULL;
...
static void _aniso_stage_cl_selftest(const int devid, void *gd_void, const dt_dev_pixelpipe_t *pipe)
{
  static int done = 0;
  ...
  // ---- CPU replica: production aniso block (COEFF_FIELD variant, solver 2) ----
  {
    ...
    // full-resolution projected polish (mirrors the production block: obstacle = clip0/L)
    {
      ...
      uint8_t *const restrict hole_flag = (uint8_t *)dt_alloc_align(region_pixels);
...
// per process.
static void _region_guided_filter_cl_selftest(const int devid, void *gd_void, const dt_dev_pixelpipe_t *pipe)
{
  static int done = 0;
...
static void _knee_cl_selftest(const int devid, void *gd_void, const dt_dev_pixelpipe_t *pipe)
{
  static int done_ = 0;
