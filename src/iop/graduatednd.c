                                ...
                                float *rotation, float *offset)
{
  ...
  float pts[4]
      = { xa * self->dev->preview_width, ya * self->dev->preview_height,
          xb * self->dev->preview_width, yb * self->dev->preview_height };
                                ...
                                float rotation, float offset)
{
  ...
  if(!dt_dev_distort_transform_plus(self->dev, self->dev->preview_pipe, self->iop_order, DT_DEV_TRANSFORM_DIR_FORW_EXCL, pts, 2))
    return 0;
  *xa = pts[0] / self->dev->preview_width;
  *ya = pts[1] / self->dev->preview_height;
  *xb = pts[2] / self->dev->preview_width;
  *yb = pts[3] / self->dev->preview_height;
  return 1;
}
