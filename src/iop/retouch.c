...
// Resolve a shape on the pipeline thread: always through the run's refcounted snapshot, never
// the GUI-owned dev->forms (see dt_dev_pixelpipe_get_roi_in() for where the snapshot comes from).
static dt_masks_form_t *rt_pipe_get_form(const dt_dev_pixelpipe_t *pipe, const int formid)
{
...
static const GList *rt_pipe_group_members(const dt_dev_pixelpipe_t *pipe, const dt_dev_pixelpipe_iop_t *piece)
{
  ...
  const dt_masks_form_t *grp = rt_pipe_get_form(pipe, bp->mask_id);
                                            ...
                                            const GList *member, int *formid, int *index)
{
  ...
  *index = rt_get_index_from_formid(p, *formid);
  ...
  return rt_pipe_get_form(pipe, *formid);
}
...
static int rt_process_forms(float *layer, dwt_params_t *const wt_p, const int scale1)
{
  ...
  // Iterate through all forms, resolved in the run's snapshot (see rt_pipe_get_form()).
