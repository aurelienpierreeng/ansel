...
#pragma omp parallel for dt_omp_default() \
    firstprivate(cnt, mean, n, roi, samples, var, col, mean_out) \
    schedule(static)
#endif
...
#ifdef _OPENMP
#pragma omp parallel for dt_omp_default() \
    firstprivate(ch, roi_out, data, in, out, hist) \
    schedule(static)
#endif
...
#ifdef _OPENMP
#pragma omp parallel for dt_omp_default() \
    firstprivate(ch, mapio, mean, roi_out, var, data, in, out) \
    schedule(static)
#endif
    for(int k = 0; k < roi_out->height; k++)
    {
...
acquire_button_pressed (GtkButton *button, dt_iop_module_t *self)
{
  ...
  dt_dev_add_history_item(darktable.develop, self, TRUE, TRUE); // FIXME: Why do we need to add this to the history?
}
...
{
  piece->data = dt_calloc_align(sizeof(dt_iop_colortransfer_data_t));
  piece->data_size = sizeof(dt_iop_colortransfer_data_t);
  dt_iop_colortransfer_data_t *d = (dt_iop_colortransfer_data_t *)piece->data;
  d->flag = NEUTRAL;
}
