...
int mouse_moved(struct dt_iop_module_t *self, double x, double y, double pressure, int which)
{
  ...
  if(g && g->draw_selected_region && g->button_down && self->enabled)
  {
    ...
    g->posx_to = pzx * self->dev->preview_width;
    g->posy_to = pzy * self->dev->preview_height;
                   ...
                   uint32_t state)
{
  ...
  if(g && g->draw_selected_region && self->enabled)
  {
    ...
    else if(which == 1)
    {
      ...
      g->posx_from = g->posx_to = pzx * self->dev->preview_width;
      g->posy_from = g->posy_to = pzy * self->dev->preview_height;
