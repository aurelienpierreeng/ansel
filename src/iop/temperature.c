...
#pragma omp parallel for dt_omp_default() \
    firstprivate(d_coeffs, height, in, out, roi_out, width, xtrans) \
    schedule(static)
#endif
...
#ifdef _OPENMP
#pragma omp parallel for dt_omp_default() \
    firstprivate(cfa_x, d_coeffs, filters, height, in, out, roi_out, width) \
    schedule(static)
#endif
    for(int j = 0; j < height; j++)
    {
...
#ifdef _OPENMP
#pragma omp parallel for dt_omp_default() \
    firstprivate(ch, d, in, out, npixels) \
    schedule(static)
#endif
...
void gui_init(struct dt_iop_module_t *self)
{
  ...
  g->finetune = dt_bauhaus_slider_new_with_range_and_feedback(darktable.bauhaus, DT_GUI_MODULE(self), -9.0, 9.0, 0, 0.0, 0, feedback);
  dt_bauhaus_widget_set_label(g->finetune, N_("finetune"));
