...
#pragma omp parallel for default(none) schedule(static)                                                           \
    dt_omp_firstprivate(normalize_C, ch, ivoid, ovoid, roi_out, display_channel) dt_omp_firstprivate(d)
#endif
...
#ifdef _OPENMP
#pragma omp parallel for default(none) dt_omp_firstprivate(normalize_C, ch, ivoid, ovoid, roi_out) dt_omp_firstprivate(d)      \
    schedule(static)
#endif
  for(size_t k = 0; k < (size_t)roi_out->width * roi_out->height; k++)
  {
                ...
                void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
  ...
  const int ch = piece->dsc_in.channels;
#ifdef _OPENMP
#pragma omp parallel for default(none) dt_omp_firstprivate(ch, ivoid, ovoid, roi_out) dt_omp_firstprivate(d) schedule(static)
#endif
  for(size_t k = 0; k < (size_t)roi_out->width * roi_out->height; k++)
  {
    float *in = (float *)ivoid + ch * k;
