...
static __thread int _hl_cl_solve_n = 0; // triangular solves
...
static inline void _hl_cl_stats_reset(void)
{
  ...
  _hl_cl_wait_n = _hl_cl_finish_n = _hl_cl_enq_n = _hl_cl_chol_n = _hl_cl_solve_n = _hl_cl_cg_n = 0;
}
...
static _sp_chol_cl_t *_sp_chol_factor_cl(const int devid, void *gd_void, const int dimension,
...
{
  dt_iop_highlights_global_data_t *global_data = (dt_iop_highlights_global_data_t *)gd_void;
  if(devid < 0 || global_data->kernel_sparse_chol_update_level < 0
     || global_data->kernel_sparse_chol_final_level < 0)
    return NULL;
  ...
  {
    const int kernel_update = global_data->kernel_sparse_chol_update_level;
    const int kernel_final = global_data->kernel_sparse_chol_final_level;
    ...
    for(int level = 0; level < factor->nlev; level++)
    {
      ...
      if(npos)
      {
        ...
        if(_hl_cl_enq2d(devid, kernel_update, sizes) != CL_SUCCESS) goto fail;
      }
      {
        ...
        if(_hl_cl_enq2dl(devid, kernel_final, sizes, local) != CL_SUCCESS) goto fail;
      }
    }
  }

  {
    ...
    dt_print(DT_DEBUG_PERF,
             "[highlights] chol factor n=%d nnz=%d nlev=%d npairs=%llu: sym=%.0fms up=%.0fms"
             " enq=%.0fms gpu=%.0fms\n",
...
// with the HL_SPCL_TEST self-test (_sp_chol_cl_selftest).
static int _sp_chol_solve_cl(const _sp_chol_cl_t *const factor, void *gd_void, cl_mem rhs)
{
  dt_iop_highlights_global_data_t *global_data = (dt_iop_highlights_global_data_t *)gd_void;
  ...
  const int kernel_fwd = global_data->kernel_sparse_chol_fwd_level;
  for(int level = 0; level < factor->nlev; level++)
  {
    const int n_level_cols = factor->lev_off[level + 1] - factor->lev_off[level];
    if(!n_level_cols) continue;
    ...
    if(_hl_cl_enq2dl(devid, kernel_fwd, sizes, local) != CL_SUCCESS) return 1;
  }

  // backward substitution (L^T x = y), one launch per backward dependency level
  const int kernel_bwd = global_data->kernel_sparse_chol_bwd_level;
  for(int level = 0; level < factor->nlev_bwd; level++)
  {
    ...
    if(_hl_cl_enq2dl(devid, kernel_bwd, sizes, local) != CL_SUCCESS) return 1;
  }
  return 0;
}
  ...
  if(unknown_count > 0)
  {

    {
      ...
      if(alloc_ok)
      {
        ...
        factor
            = _sp_chol_factor_cl(devid, gd_void, unknown_count, matrix_col_ptr, matrix_row_index, matrix_values);
        ...
        if(factor)
        {
          ...
          if(rhs_device && !_sp_chol_solve_cl(factor, gd_void, rhs_device)
                                   ...
                                   const float reg_radius, const int extent, const dt_dev_pixelpipe_t *pipe)
{
  ...
  {
    const double _tch = dt_get_wtime();
    factor = _sp_chol_factor_cl(devid, gd_void, n_unknowns, matrix_col_ptr, matrix_row_index, matrix_values);
    _hl_cl_chol_s += dt_get_wtime() - _tch;
    _hl_cl_chol_n++;
  ...
  for(int c = 0; c < 3; c++)
  {
    ...
    if(!use_cg)
    {
      if(_sp_chol_solve_cl(factor, gd_void, rhs_dev))
      {
        cl_err = DT_OPENCL_DEFAULT_ERROR;
        goto out;
      }
  ...
  *corr_out = NULL;
  ...
  if(cl_err == CL_SUCCESS)
  {
    ...
    dt_print(DT_DEBUG_PERF,
             ...
             " chol=%d (%.0f ms) solves=%d cg_iters=%d\n",
             _hl_cl_wait_n, _hl_cl_wait_s * 1e3, _hl_cl_finish_n, _hl_cl_finish_s * 1e3, _hl_cl_enq_n,
             _hl_cl_chol_n, _hl_cl_chol_s * 1e3, _hl_cl_solve_n, _hl_cl_cg_n);
  }
