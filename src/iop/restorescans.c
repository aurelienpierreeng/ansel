...
#ifdef _OPENMP
#pragma omp parallel for default(firstprivate) schedule(static) firstprivate(ivoid, ovoid, roi_in, roi_out, in, out, cmy)
#endif
...
#ifdef _OPENMP
#pragma omp parallel for default(firstprivate) schedule(static) firstprivate(d, temp_in, roi_in, roi_out)
#endif
    for(int i = 0; i < roi_out->height; i++)
      for(int j = 0; j < roi_out->width; j++)
      {
...
#ifdef _OPENMP
#pragma omp parallel for default(firstprivate) schedule(static) firstprivate(d, temp_in, temp_out, roi_in, roi_out, sharpen)
#endif
    for(int i = 0; i < roi_out->height; i++)
      for(int j = 0; j < roi_out->width; j++)
      {
...
#ifdef _OPENMP
#pragma omp parallel for default(firstprivate) schedule(static) firstprivate(roi_in, roi_out, out, temp_out)
#endif
  for(int i = 0; i < roi_out->height; i++)
    for(int j = 0; j < roi_out->width; j++)
    {
