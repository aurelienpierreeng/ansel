...
#ifdef _OPENMP
#pragma omp parallel for simd firstprivate(in, out, buffsize, alpha, mask_color) \
...
#ifdef _OPENMP
#pragma omp parallel for firstprivate(in, out, buffsize, alpha, mask_color) schedule(static)
#endif
      for(size_t j = 0; j < buffsize; j += 4)
      {
...
{
#ifdef _OPENMP
#pragma omp parallel for simd firstprivate(in, out, buffsize) \
