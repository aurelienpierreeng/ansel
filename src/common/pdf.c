...
float * read_ppm(const char * filename, int * wd, int * ht)
{
  ...
  if(max <= 255)
  {
    ...
    #ifdef _OPENMP
    #pragma omp parallel for schedule(static) dt_omp_default() firstprivate(image, tmp, width, height, max)
    #endif
  ...
  else
  {
    ...
    // swap byte order
    #ifdef _OPENMP
    #pragma omp parallel for schedule(static) dt_omp_default() firstprivate(tmp, width, height)
    #endif
    for(int k = 0; k < 3 * width * height; k++)
      tmp[k] = ((tmp[k] & 0xff) << 8) | (tmp[k] >> 8);
    // and transform it into 0..1 range
    #ifdef _OPENMP
    #pragma omp parallel for schedule(static) dt_omp_default() firstprivate(image, tmp, max, width, height)
    #endif
    for(int i = 0; i < width * height * 3; i++)
      image[i] = (float)tmp[i] / max;
    dt_free(tmp);
...
#ifdef _OPENMP
  #pragma omp parallel for schedule(static) dt_omp_default() firstprivate(image, data, width, height)
#endif
    for(int i = 0; i < width * height * 3; i++)
