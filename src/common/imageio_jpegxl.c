                                           ...
                                           dt_mipmap_buffer_t *mbuf)
{
  ...
  if(!inputfile)
  {
    fprintf(stderr, "[imageio_open_jpegxl] cannot open file for read: '%s'\n",
             filename);
  ...
  {
    fprintf(stderr, "[imageio_open_jpegxl] failed to read entire file (%zu bytes) from '%s'\n",
             inputFileSize,
             filename);
    free(read_buffer);
    fclose(inputfile);
  ...
  if(!decoder)
  {
    fprintf(stderr, "[imageio_open_jpegxl] JxlDecoderCreate failed\n");
    free(read_buffer);
    return DT_IMAGEIO_LOAD_FAILED;
  }
  ...
  if(!runner)
  {
    fprintf(stderr, "[imageio_open_jpegxl] JxlResizableParallelRunnerCreate failed\n");
  ...
  {
    fprintf(stderr, "[imageio_open_jpegxl] JxlDecoderSetInput failed\n");
  ...
  if(JxlDecoderSubscribeEvents(decoder,
     ...
     != JXL_DEC_SUCCESS)
  {
    fprintf(stderr, "[imageio_open_jpegxl] JxlDecoderSubscribeEvents failed\n");
    JxlResizableParallelRunnerDestroy(runner);
    JxlDecoderDestroy(decoder);
    free(read_buffer);
    return DT_IMAGEIO_LOAD_FAILED;
  ...
  if(JxlDecoderSetParallelRunner(decoder, JxlResizableParallelRunner, runner)
     != JXL_DEC_SUCCESS)
  {
    fprintf(stderr, "[imageio_open_jpegxl] JxlDecoderSetParallelRunner failed\n");
    JxlResizableParallelRunnerDestroy(runner);
    JxlDecoderDestroy(decoder);
    free(read_buffer);
    return DT_IMAGEIO_LOAD_FAILED;
  ...
  while(1)
  {
    ...
    {
      fprintf(stderr, "[imageio_open_jpegxl] JXL decoding failed\n");
      JxlResizableParallelRunnerDestroy(runner);
      JxlDecoderDestroy(decoder);
      free(read_buffer);
    ...
    {
      fprintf(stderr, "[imageio_open_jpegxl] JXL data incomplete\n");
      JxlResizableParallelRunnerDestroy(runner);
      JxlDecoderDestroy(decoder);
      free(read_buffer);
      return DT_IMAGEIO_FILE_CORRUPTED;
    ...
    if(status == JXL_DEC_BASIC_INFO)
    {
      ...
      {
        fprintf(stderr, "[imageio_open_jpegxl] JXL basic info not available\n");
        JxlResizableParallelRunnerDestroy(runner);
        JxlDecoderDestroy(decoder);
        free(read_buffer);
        return DT_IMAGEIO_FILE_CORRUPTED;
      ...
      {
        fprintf(stderr, "[imageio_open_jpegxl] JXL image declares zero dimensions\n");
        JxlResizableParallelRunnerDestroy(runner);
        JxlDecoderDestroy(decoder);
        free(read_buffer);
        return DT_IMAGEIO_FILE_CORRUPTED;
    ...
    if(status == JXL_DEC_COLOR_ENCODING)
    {
      ...
      } else
      {
        ...
        fprintf(stderr, "[imageio_open_jpegxl] the image '%s' has an unknown or XYB "
    ...
    if(status == JXL_DEC_NEED_IMAGE_OUT_BUFFER)
    {
      ...
      if(!mipbuf)
      {
        ...
        JxlDecoderDestroy(decoder);
        free(read_buffer);
        fprintf(stderr, "[imageio_open_jpegxl] could not alloc full buffer for image: '%s'\n",
                 img->filename);
    ...
    // perhaps the user intends to use darktable as a DAM for such images.
