...
#pragma omp parallel for dt_omp_default() \
  dt_omp_firstprivate(height, st, step, width, ch, buf) \
  schedule(static)
#endif
...
static void dt_focus_draw_clusters(cairo_t *cr, int width, int height, int32_t imgid, int buffer_width,
                                   ...
                                   float full_zoom, float full_x, float full_y)
{
  ...
  // could use dt_image_altered() here, but it ignores flip module
  {
    ...
    if(res)
    {
      ...
      dt_dev_pixelpipe_set_input(&pipe, &dev, UNKNOWN_IMAGE, wd, ht, 1.0f, DT_MIPMAP_NONE);
