                                        ...
                                        const gboolean valid_exif, dt_image_t *img)
{
     ...
     && !dt_imageio_large_thumbnail(filename, &buffer, &th_width, &th_height, &color_space, width, height))
  {
    ...
    {
#ifdef _OPENMP
#pragma omp parallel for dt_omp_default() dt_omp_firstprivate(th_width, th_height, buffer, rgb) schedule(static)
#endif
...
#ifdef _OPENMP
#pragma omp parallel for dt_omp_default() dt_omp_firstprivate(in, pixels, rgb) schedule(static)
#endif
