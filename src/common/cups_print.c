/*
    ...
    Copyright (C) 2014-2015, 2017-2018, 2020-2021 Pascal Obry.
    Copyright (C) 2015 Jérémy Rosen.
    Copyright (C) 2015, 2019-2020 parafin.
    Copyright (C) 2015-2016 Roman Lebedev.
    Copyright (C) 2015-2018 Tobias Ellinghaus.
    Copyright (C) 2017-2018, 2022 Dan Torop.
...
GList *dt_get_papers(const dt_printer_info_t *printer)
{
  ...
  if (!IS_NULL_PTR(&cupsConnectDest) && !IS_NULL_PTR(&cupsCopyDestInfo) && !IS_NULL_PTR(&cupsGetDestMediaCount) &&
      !IS_NULL_PTR(&cupsGetDestMediaByIndex) && !IS_NULL_PTR(&cupsFreeDestInfo))
#endif
  {
    cups_dest_t *dests;
    const int num_dests = cupsGetDests(&dests);
