...
#pragma omp parallel for simd aligned(in, out : 16) dt_omp_default() \
    firstprivate(in, out, nfloats) schedule(simd:static)
...
#pragma omp parallel for simd aligned(buf, src : 16) dt_omp_default() \
  firstprivate(buf, src, scale, nfloats) schedule(simd:static)
    for(size_t k = 0; k < nfloats; k++)
      buf[k] = scale * src[k];
    return;
  }
...
#pragma omp parallel for dt_omp_default() \
  firstprivate(buf, fill_value, nfloats, nthreads, chunksize) schedule(static) num_threads(nthreads)
...
#pragma omp parallel for simd aligned(buf:16) dt_omp_default() \
  firstprivate(buf, add_value, nfloats) schedule(simd:static)
    for(size_t k = 0; k < nfloats; k++)
      buf[k] += add_value;
    return;
  }
...
#pragma omp parallel for simd aligned(buf, other_image : 16) dt_omp_default() \
  firstprivate(buf, other_image, nfloats) schedule(simd:static)
    for(size_t k = 0; k < nfloats; k++)
      buf[k] += other_image[k];
    return;
  }
                         ...
                         const size_t ch)
{
  ...
  if (nfloats > parallel_imgop_minimum)	// is the copy big enough to outweigh threading overhead?
  {
    ...
    // memory won't be able to take advantage of more than four cores).
#pragma omp parallel for simd aligned(buf:16) dt_omp_default() \
  firstprivate(buf, max_value, nfloats) schedule(simd:static)
    for(size_t k = 0; k < nfloats; k++)
                            ...
                            const size_t ch)
{
  ...
  if (nfloats > parallel_imgop_minimum)	// is the copy big enough to outweigh threading overhead?
  {
    ...
    // memory won't be able to take advantage of more than four cores).
#pragma omp parallel for simd aligned(buf:16) dt_omp_default() \
  firstprivate(buf, mul_value, nfloats) schedule(simd:static)
    for(size_t k = 0; k < nfloats; k++)
      buf[k] *= mul_value;
    return;
  }
                            ...
                            const size_t ch)
{
  ...
  if (nfloats > parallel_imgop_minimum)	// is the copy big enough to outweigh threading overhead?
  {
    ...
    // memory won't be able to take advantage of more than four cores).
#pragma omp parallel for simd aligned(buf:16) dt_omp_default() \
  firstprivate(buf, div_value, nfloats) schedule(simd:static)
    for(size_t k = 0; k < nfloats; k++)
      buf[k] /= div_value;
    return;
  }
                               ...
                               const size_t width, const size_t height, const size_t ch)
{
  ...
  if (nfloats > parallel_imgop_minimum/2) // is the task big enough to outweigh threading overhead?
  {
    ...
    // memory won't be able to take advantage of more than four cores).
#pragma omp parallel for simd aligned(buf:16) dt_omp_default() \
  firstprivate(buf, lambda, lambda_1,  nfloats, other) schedule(simd:static)
    for(size_t k = 0; k < nfloats; k++)
