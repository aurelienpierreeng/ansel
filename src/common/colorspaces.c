...
#ifdef _OPENMP
#pragma omp parallel for dt_omp_default() firstprivate(transform) firstprivate(image_in, image_out, width, height) schedule(static)
#endif
...
#ifdef _OPENMP
#pragma omp parallel for dt_omp_default() firstprivate(in, out, num, RGB_to_RGB_WB) schedule(static)
#endif
  for(int i = 0; i < num; i++)
  {
...
{
#ifdef _OPENMP
#pragma omp parallel for dt_omp_default() firstprivate(out, num, CAM_to_RGB) schedule(static)
#endif
  for(int i = 0; i < num; i++)
  {
    float *in = &out[i*4];
...
{
#ifdef _OPENMP
#pragma omp parallel for dt_omp_default() firstprivate(out, num, RGB_to_CAM) schedule(static)
#endif
  for(int i = 0; i < num; i++)
  {
    float *in = &out[i*3];
