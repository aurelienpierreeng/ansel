...
deep-copied" above) — through `rt_pipe_get_form()`, `rt_pipe_group_members()` and
`rt_pipe_member_form()`, and read the group id from `piece->blendop_data`, never from
`self->blend_params`. The CPU and OpenCL callbacks share their whole per-shape preamble (lookup,
